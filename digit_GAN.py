...
def train_loop(dataloader, G, D, loss, G_optimizer, D_optimizer, G_loss, D_loss, counter, epochIdx, device):
    ...
    for batchIdx, (data, target) in enumerate(dataloader):
        ...
        #forward
        generated_data = G(noise).to(device) #batch_size x 784
        ...
        D_true_output = D(true_data).to(device).view(batch_size)
        # Compute Loss
        D_true_loss = loss(D_true_output, true_labels)
        # Forward pass with generated data as input
        D_generated_output = D(generated_data.detach()).to(device).view(batch_size)
        # Compute Loss 
        ...
        D_generated_output = D(generated_data).to(device).view(batch_size)
        # Compute loss
        ...
        if ((batchIdx + 1)% 500 == 0 and (epochIdx + 1)%2 == 0):
            ...
            with torch.no_grad():
                noise = torch.randn(batch_size,100).to(device)
                generated_data = G(noise).view(batch_size, 28, 28)
...
def main(argv):
    ...
    with torch.no_grad():
        ...
        generated_data = generator(noise).view(9, 28, 28)
